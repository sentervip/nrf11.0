...
#define BTN_ID_RIGHT				2
#define BTN_ID_SELECT				3
#define BTN_ID_DOWN					4

#define BTN_ACTION_PRESS			BSP_BUTTON_ACTION_PUSH
...
static uint32_t cfgMultiboardBtns(void)
{
	uint32_t err_code;

    err_code = bsp_event_to_button_action_assign(BTN_ID_UP,
                                                 BTN_ACTION_PRESS,
                                                 BSP_EVENT_UP);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);

    err_code = bsp_event_to_button_action_assign(BTN_ID_LEFT,
                                                 BTN_ACTION_PRESS,
                                                 BSP_EVENT_LEFT);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);

    err_code = bsp_event_to_button_action_assign(BTN_ID_RIGHT,
                                                 BTN_ACTION_PRESS,
                                                 BSP_EVENT_RIGHT);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);

	err_code = bsp_event_to_button_action_assign(BTN_ID_SELECT,
                                                 BTN_ACTION_PRESS,
                                                 BSP_EVENT_SELECT);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);

	err_code = bsp_event_to_button_action_assign(BTN_ID_DOWN,
                                                 BTN_ACTION_PRESS,
                                                 BSP_EVENT_DOWN);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);
...
uint32_t bsp_btn_ble_init(bsp_btn_ble_error_handler_t error_handler, bsp_event_t * p_startup_bsp_evt)
{
    ...
    if (m_num_connections == 0)
    {
		//replace by rf_smart,as we need to redefine button function
        //err_code = advertising_buttons_configure();
        err_code = cfgMultiboardBtns();
    }
